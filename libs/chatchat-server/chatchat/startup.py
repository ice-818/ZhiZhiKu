...
def dump_server_info(after_start=False, args=None):
    ...
    from chatchat.branding import PROJECT_NAME_CN, startup_banner
    from chatchat.settings import Settings
    ...
    if after_start:
        ...
        print(f"服务端运行信息：")
        if args.api:
            print(f"    {PROJECT_NAME_CN} Api Server: {api_address()}")
        if args.webui:
            print(f"    {PROJECT_NAME_CN} WEBUI Server: {webui_address()}")
    print("=" * 30 + banner + "=" * 30)
    print("\n")
